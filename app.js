...
Write a function called sum() that takes in two numbers as arguments and then returns an array
 where the first element is the sum of those numbers, and the second element is a concatenated string that EXACTLY follows this example and uses the values that were input into the function:
...
function sumAndMultiply(a, b, c) {
  ...
  // console.log([sum(sum(a,b)[0],c)[0] , multiply(multiply(a,b)[0],c )[0] ,a+' and '+b+' and '+c+' sum to '+ sum(sum(a,b),c) +'.','The product of '+a+' and '+b+' and '+c+' is '+multiply(multiply(a,b),c )+'.']);
...
for(let i=0;i<dynamicArray.length;i++){
  mosab=multiply(dynamicArray[i],mosab)[0]
  oldStrings=sum(oldStrings,dynamicArray[i])[0]
  if(i<dynamicArray.length-1){
  oldStrings=sum(oldStrings,',')[0];
  }
}
oldStrings='The numbers '+oldStrings+' have a product of '+mosab+'.'
return [mosab, oldStrings];
}
